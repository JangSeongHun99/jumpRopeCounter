...
합성 랜드마크로 점프와 어깨 으쓱을 만들어 점프만 세는지 확인한다.
...
def make_pose(shoulder_dy: float = 0.0, body_dy: float = 0.0, hips_visible: bool = True):
    """정규화 좌표의 33개 랜드마크. body_dy는 전신이 위로 움직인 양, shoulder_dy는 어깨만 움직인 양."""
    pts = [Landmark(0.5, 0.5, 0.0)] * 33

    def put(i, x, y, v=0.95):
        pts[i] = Landmark(x, y - body_dy, v)
    ...
    put(24, 0.55, 0.62, hv)
    put(27, 0.45, 0.95, 0.9)                  # 발목
    put(28, 0.55, 0.95, 0.9)
    return pts


def bumps(n: int, amp: float, kind: str, hips_visible: bool = True,
          period_s: float = 0.8, bump_s: float = 0.4):
    """n번의 반정현파 상승/하강 (kind: "jump" = 전신, "shrug" = 어깨만)."""
    frames = []
    ...
    for i in range(total):
        ...
        frames.append(make_pose(shoulder_dy=d if kind == "shrug" else 0.0,
                                body_dy=d if kind == "jump" else 0.0,
...
def main() -> int:
    cases = [
        ...
        ("점프 5회 (상반신만 보임)", bumps(5, 0.04, "jump", hips_visible=False), 5),
        ("작은 점프 5회 (진폭 몸통의 7%)", bumps(5, 0.015, "jump"), 5),
        ("어깨 으쓱 5회 (전신 보임)", bumps(5, 0.04, "shrug"), 0),
        ("어깨 으쓱 5회 (상반신만 보임)", bumps(5, 0.04, "shrug", hips_visible=False), 0),
