...
7. 제자리에서 뛰었는지 확인합니다. 최저점에서 최고점 사이에 좌우로 몸통 길이의 절반 넘게 움직였거나(걷기),
   몸통 크기가 15% 넘게 변했으면(앞으로 숙이기, 카메라 쪽으로 기울이기) 점프가 아닙니다.
...
- 카메라와 2~3 m 거리를 두세요. 가까이 앉아서 몸을 앞뒤로 흔들면 몸통 크기 변화로 걸러지지만,
  의자에서 몸을 튕기는 동작처럼 몸 전체가 리듬 있게 오르내리면 구분할 수 없습니다.
- 처음 3회는 리듬이 확인될 때까지 화면에 `starting n/3`으로만 표시되고, 3회째에 한꺼번에 반영됩니다.
- 그래도 잘못 세면 `-`로 민감도를 낮추세요 (threshold 0.05 -> 0.06...). 낮게 뛰는데 안 세면 `+`.
