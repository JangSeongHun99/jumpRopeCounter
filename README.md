...
9. 줄에 걸린 시도를 뺍니다. 줄은 30fps 웹캠에 보이지 않아 직접 확인할 수 없지만, 걸리면 반드시 리듬이
   끊깁니다. 2초 넘게 쉬었다가 다시 리듬이 잡히면 그 직전 점프(걸린 시도)를 1회 빼고 `miss`를 1 올리며,
   최고 연속 기록(`best`)도 따로 셉니다. 누적 카운트는 유지되고 이어서 올라갑니다.
...
- 줄에 걸린 것은 "리듬 끊김"으로만 알아채므로, 걸렸는데 2초 안에 바로 이어서 뛰면 그 시도가 세어집니다.
