...
8. 리듬을 확인합니다. 줄넘기는 일정한 간격(분당 50~240회)으로 반복되므로, 후보 점프가 비슷한 간격으로
   3회 이어졌을 때 비로소 세기 시작하고 앞의 2회도 그때 한꺼번에 반영합니다(화면에 `starting 1/3`,
   `2/3`이 표시됨). 리듬이 잡힌 뒤에는 바로바로 세고, 2초 넘게 쉬면 다시 3회부터 확인합니다.
   자세 바꾸기, 몸 흔들기, 한두 번 튕기기 같은 산발적 움직임은 여기서 걸러집니다.
